...
public class NodePatternClause extends Clause {
    ...
    //生成一个NodePatternClause 不新增加变量用于where匹配
    public static NodePatternClause generateRandomNodePatternDontNewNode(AbstractNode node, GraphManager graphManager) {
        ...
        List<String> existingNodeVariables = new ArrayList<>(nodeVariableManager.getAllNodeVariables());
        if (!existingNodeVariables.isEmpty())
            nodeVariable = existingNodeVariables.get(randomly.getInteger(0,existingNodeVariables.size()));
