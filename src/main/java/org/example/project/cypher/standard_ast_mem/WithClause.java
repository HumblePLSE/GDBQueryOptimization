...
public class WithClause extends ProjectingClause {
    ...
    public WithClause(List<ContextVariable> contextVariables, boolean useWildcard,WhereClause whereClause, List<ReadingSubClause> readingSubClauses,boolean distinct) {
        super("WITH");
    ...
    public String toCypher() {
        ...
        if (contextVariables==null) return "";
        sb.append("WITH ");
    ...
    public static WithClause generateRandomWithClauseNew(GraphManager graphManager) {
        ...
        //if(useWildcard) return new WithClause(null,true,null,null);
        ...
        if (nodeVariables.isEmpty() && relationshipVariables.isEmpty() && pathVariables.isEmpty()&&aliasVariables.isEmpty()) {
            return new WithClause(null,true,null,null,distinct);  //
        }
        //if(useWildcard) return new WithClause(new ArrayList<>(),true,null,null);
        ...
        contextVariables.addAll(createAliasesContextVariables(selectedAsVariables,graphManager,useWildcard));
        if (!useWildcard) {
        ...
        if(randomly.getBoolean()){
            ...
            //TODO where里按模式匹配
//            whereClause = WhereClause.generateRandomWhereClauseChose(graphManager,varToProperties);

            whereClause = WhereClause.generateRandomWhereClause(combinedProperties);
        }

        return new WithClause(contextVariables, useWildcard, whereClause,readingSubClauses,distinct); // Return the selected variables
    ...
    public static WithClause generateRandomWithClause(GraphManager graphManager) {
        ...
        if (useWildcard) {
            return new WithClause(null, true, null,null,distinct); // Use wildcard *
        } else {
            List<ContextVariable> contextVariables = new ArrayList<>();
            ...
            return new WithClause(contextVariables, false,null, readingSubClauses,distinct); // Return the selected variables
        }
    }
    ...
    private static List<ContextVariable> createAliasesContextVariables(List<String> aliasVariables, GraphManager graphManager,boolean useWildcard) {
        Randomly randomly = new Randomly();
        List<ContextVariable> contextVariables = new ArrayList<>();
       if(!useWildcard) {
    ...
    private static List<String> extractRandomNodeAndRelationshipAttributes(List<ContextVariable> contextVariables, GraphManager graphManager) {
        ...
        for (String nodeVariable : nodeVariables) {
            ...
            for (int i = 0; i < Math.min(selectPropertynum, availableProperties.size()); i++) {
                IPropertyInfo property = availableProperties.get(i);
                if(property.getType()!=CypherType.LIST) attributes.add(nodeVariable + "." + property.getKey());  // 拼接成 nodeVariable.propertyname 形式
            }
        }
        ...
        for (String relationshipVariable : relationshipVariables) {
            ...
            for (int i = 0; i < Math.min(selectPropertynum, availableProperties.size()); i++) {
                IPropertyInfo property = availableProperties.get(i);
                if(property.getType()!=CypherType.LIST) attributes.add(relationshipVariable + "." + property.getKey());  // 拼接成 relationshipVariable.propertyname 形式
            }
        }
