...
public class SimplePathPatternClause extends Clause {
    ...
    public static SimplePathPatternClause generateRandomSimplePathPatternInWhere(GraphManager graphManager,Set<AbstractRelationship> visitedRelationships) {
        // 随机选择起始节点
        ...
        // 100% 概率从 NodeVariableManager 中获取随机节点变量

        List<AbstractNode> nodesWithRelationships = graphManager.getNodes().stream()
                .filter(node -> !node.getRelationships().isEmpty())
                .collect(Collectors.toList());

        startNode = nodesWithRelationships.get(randomly.getInteger(0,nodesWithRelationships.size()));
        startNodePattern = NodePatternClause.generateRandomNodePatternDontNewNode(startNode,graphManager.Copy());
        ...
        while (true) {
            ...
            while (leftPath.isEmpty() && rightPath.isEmpty() && direction.equals("none")){
                direction = getRandomDirection(0);
            }
            ...
            if (nodeVariable != null && randomly.getInteger(0, 100) < 80) {
                nextNodePattern = NodePatternClause.generateVariablePattern(nodeVariable);
            } else {
                nextNodePattern = NodePatternClause.generateRandomNodePatternDontNewNode(nextNode, graphManager);
            }

            String direc = relationship.getFrom().equals(currentNode) ? "left" : "right";
            RelationshipPatternClause relationshipPatternClause=RelationshipPatternClause.generateRandomRelationshipPatternDontNewRe(relationship,direc,graphManager);
        ...
        while (true) {
            ...
            rightPath.add(new RelationshipNodePair(RelationshipPatternClause.generateRandomRelationshipPatternDontNewRe(relationship, direc, graphManager), nextNodePattern));
    ...
    public static SimplePathPatternClause generateRandomSimplePathPattern(GraphManager graphManager,Set<AbstractRelationship> visitedRelationships) {
        ...
        while (true) {
            ...
            NodePatternClause nextNodePattern;
            if (nodeVariable != null && randomly.getInteger(0, 100) < 70) {
    ...
    private static String getRandomDirection(int directionchoice) {
        Randomly randomly = new Randomly();
        int rand = randomly.getInteger(directionchoice,5); // 0 = any, 1 = to, 2 = from, 3 = none
        switch (rand) {
            case 0:
                return "any";
            case 1:
